...
export default function EventDashboard() {
    ...
    if (pageload) return <div className="bg-default"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 gap-4 w-[120vw] h-[120vh] z-10 bg-black/10"><Loader /></div></div>;
            ...
            <div className="w-full h-screen mx-auto relative z-1">
                ...
                <div className={`flex flex-col justify-center text-stone-700 p-3 glass-bg shadow-lg shadow-black/10 text-left transition-all duration-300 ease-in-out ${expanded ? "rounded-t-none" : "rounded-none"}`}>
                    ...
                    <div className={`text-left text-xs sm:text-sm transition-all duration-300 ease-in-out ${expanded ? "max-h-96 opacity-100 translate-y-0" : "max-h-0 opacity-0 -translate-y-2"}`}>
                        <div className="border-b border-black/10 w-[98vw] sm:self-start my-2 relative z-1 left-1/2 -translate-x-1/2"></div>
                        ...
                        <span className="flex items-top relative mb-1"><FiCalendar className="absolute top-1 size-4"/><p className="pl-8 relative top-1">{new Date(event.date).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p></span>
                    </div>
                </div>
